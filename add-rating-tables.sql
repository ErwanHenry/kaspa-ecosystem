-- Script pour ajouter les tables de ratings et commentaires
...
CREATE POLICY "Allow users to update own ratings" ON ratings
    FOR UPDATE USING (user_wallet = current_setting('request.jwt.claims', true)::json->>'wallet');
...
BEGIN
    UPDATE projects
    SET 
        rating_avg = (SELECT AVG(rating) FROM ratings WHERE project_id = NEW.project_id),
        rating_count = (SELECT COUNT(*) FROM ratings WHERE project_id = NEW.project_id),
        average_rating = (SELECT AVG(rating) FROM ratings WHERE project_id = NEW.project_id)
...
BEGIN
    UPDATE projects
    SET comment_count = (SELECT COUNT(*) FROM comments WHERE project_id = COALESCE(NEW.project_id, OLD.project_id))
