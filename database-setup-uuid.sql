-- Script de configuration de la base de données Kaspa Ecosystem (Version UUID)
...
-- 3. Si la table projects existe déjà avec une colonne 'category' (texte),
...
-- Vérifier d'abord si la colonne 'category' existe
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        UPDATE projects SET category_id = 
            CASE 
                WHEN category = 'Wallets' OR category = 'wallets' THEN 1
                WHEN category = 'Mining' OR category = 'mining' THEN 2
                WHEN category = 'DeFi' OR category = 'defi' THEN 3
                WHEN category = 'NFTs' OR category = 'nfts' THEN 4
                WHEN category = 'Infrastructure' OR category = 'infrastructure' THEN 5
                WHEN category = 'Education' OR category = 'education' THEN 6
                WHEN category = 'Tools' OR category = 'tools' THEN 7
                ELSE 8 -- Other
            END
...
-- 6. Créer la table sponsorships avec UUID pour project_id
...
BEGIN
    ...
    -- Ajouter logo_url si manquant
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'projects' AND column_name = 'logo_url') THEN
        ALTER TABLE projects ADD COLUMN logo_url TEXT;
    END IF;
    
    -- Ajouter website si manquant
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'projects' AND column_name = 'website') THEN
...
DROP POLICY IF EXISTS "Allow public read sponsorships" ON sponsorships;

-- 10. Créer les nouvelles politiques
