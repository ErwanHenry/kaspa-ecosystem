...
window.KaspaEcosystemAPI = {
    ...
    async loadFeaturedProjects() {
        try {
            // Charger les projets avec sponsorships actifs
            const { data, error } = await supabaseClient
                .from('projects')
                .select(`
                    *,
                ...
                .gt('sponsorships.end_date', new Date().toISOString())
            ...
            if (data && data.length > 0) {
                ...
                this.renderCarousel();
                document.getElementById('featured-carousel').style.display = 'block';
                document.getElementById('no-featured').style.display = 'none';
            } else {
                // Aucun projet sponsorisé
                document.getElementById('featured-carousel').style.display = 'block';
                document.getElementById('no-featured').style.display = 'block';
    ...
    renderCarousel() {
        ...
        track.innerHTML = this.featuredProjects.map(project => {
            const highestBid = project.sponsorships[0].bid_amount;
            return `
                <div class="carousel-slide">
                    <div class="featured-card">
                        <div class="sponsored-badge">SPONSORED</div>
                        <img src="${project.logo_url || '/images/kaspa-logo.png'}" 
                             alt="${project.title}" 
                             onerror="this.src='/images/kaspa-logo.png'">
                        <h3>${project.title}</h3>
                        <p>${project.description ? project.description.substring(0, 100) + '...' : 'No description available'}</p>
                        <div class="bid-info">
                            <span class="bid-amount">${highestBid} KAS</span>
                        </div>
                        <div class="project-actions">
                            <button class="btn btn-primary" onclick="app.showProjectDetails('${project.id}')">View Details</button>
                            ${project.website ? `<a href="${project.website}" target="_blank" class="btn btn-secondary">Visit Site</a>` : ''}
                        </div>
                    </div>
                </div>
            `;
    ...
    moveCarousel(direction) {
        ...
        if (this.carouselIndex < 0) {
            this.carouselIndex = Math.max(0, this.featuredProjects.length - 3);
        }
        if (this.carouselIndex > this.featuredProjects.length - 3) {
            this.carouselIndex = 0;
        }
    ...
    updateCarouselPosition() {
        ...
        const slideWidth = 320; // largeur d'une slide + gap
        track.style.transform = `translateX(-${this.carouselIndex * slideWidth}px)`;
    }

    // Ajouter cette fonction au init()
