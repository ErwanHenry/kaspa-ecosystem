...
window.KaspaSponsorshipSystem = KaspaSponsorshipSystem;
    ...
    // Report Scam avec déclenchement automatique d'alerte
    async reportScam(projectId, reason) {
        if (!app.walletManager?.isConnected()) {
            app.showToast('Please connect your wallet first', 'error');
            return;
        }
        ...
        try {
            ...
            // Récupérer le nombre total de reports
            ...
            const { data: settings } = await supabaseClient
                .from('admin_settings')
                .select('value')
                .eq('key', 'scam_threshold')
                .single();
            
            const threshold = parseInt(settings?.value || '5');
            
            // Déclencher l'alerte si le seuil est atteint
            if (count >= threshold) {
                try {
                    ...
                    console.log('Scam alert sent for project:', projectId);
                } catch (alertError) {
                    console.error('Failed to send alert:', alertError);
                }
            }
            
            app.showToast('Report submitted successfully', 'success');
            
            // Fermer le modal
            document.querySelector('.modal')?.remove();
            
            // Rafraîchir l'affichage du projet
    ...
    // Modal pour reporter un scam
    showReportScamModal(projectId, projectTitle) {
        const modal = document.createElement('div');
        modal.className = 'modal show';
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 500px;">
                <div class="modal-header">
                    <h2 class="modal-title">⚠️ Report Scam: ${projectTitle}</h2>
                    <button class="close-btn" onclick="this.closest('.modal').remove()">&times;</button>
                </div>
                <div class="modal-body">
                    <div class="warning-box">
                        <p><strong>Important:</strong> False reports may result in penalties.</p>
                        <p>Please provide specific details about why you believe this project is a scam.</p>
                    </div>
                    
                    <form onsubmit="app.sponsorshipSystem.submitScamReport(event, '${projectId}')">
                        <div class="form-group">
    ...
    async submitScamReport(event, projectId) {
        event.preventDefault();
        const formData = new FormData(event.target);
        const reason = formData.get('reason');
        await this.reportScam(projectId, reason);
    }
